...
\section{数据来源以及初步分析}
\subsection{数据来源}
GEO数据库(Gene Expression Omnibus)是由美国国家生物信息中心(National Center for Biotechnology Information)提供及维护的公共基因表达数据资源. 通过对GEO数据库上某个实验相关样本数据集系列(Series)进行分析, 就有机会获得对基因间关系的深入认知, 以实现疾病的辅助治疗.

由于队伍成员现有设备运算能力的限制, 在处理单个系列时就需要耗费大量时间, 最后我们决定暂时只以GSE81089为数据来源, 计算非小细胞肺癌(non-small cell lungcancer)的基因关联网络.
\subsection{疾病介绍}
肺癌是世界上最常见的恶性肿瘤之一, 并且肺癌发生过程涉及的分子机制仍未被全部解开. 非小细胞肺癌包括鳞状细胞癌, 腺癌, 大细胞癌, 与小细胞癌相比其癌细胞生长分裂较慢, 扩散转移相对较晚. 非小细胞肺癌约占所有肺癌的约75\%的患者发现时已处于中晚期, 5年生存率很低. 而在非小细胞肺癌中, 病变部位的组织学分类决定了患者治疗的方式. 而通过基因关联……
\subsection{初步分析}
